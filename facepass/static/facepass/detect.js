...
class FaceDetector {
    ...
    constructor(media) {
        ...
        const MODEL_URL = 'static/facepass/models';
    ...
    */
    ...
    /* 
    * Method that stops the stream
    */
    async stopStream() {
        const stream =  this.media.srcObject;
    ...
    display(message, output) {
        ...
        outputElement = document.querySelector(`#${output}`) || null;
        ...
        return Promise.all(
            ...
            labels.map(async (label, key) => {
                ...
                for (let i = 1; i <= sampleSize; i++) {
                    ...
                    if(images[0].constructor === Array) {
                        url =  images[key].length > 0 ? images[key][i-1] : `static/facepass/recognition/${label}/${i}.png`;
                    } else {
                        url =  images.length > 0 ? images[i-1] : `static/facepass/recognition/${label}/${i}.png`;
                    }
